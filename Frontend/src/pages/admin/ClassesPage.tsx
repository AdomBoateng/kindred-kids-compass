...
export default function ClassesPage() {
  ...
  return <Layout><PageHeader title="Classes" description="Manage classes" /><div className="flex justify-between mb-4"><Input placeholder="Search classes" value={searchTerm} onChange={(e)=>setSearchTerm(e.target.value)} className="w-full max-w-sm" /><Button asChild><Link to="/admin/classes/new"><Plus className="mr-2 h-4 w-4" />Create Class</Link></Button></div><div className="grid gap-4 md:grid-cols-2">{filtered.map((c)=><Card key={c.id} className="border-0 shadow-xl bg-gradient-to-br from-violet-600 to-indigo-600 text-white"><CardContent className="pt-6 flex items-center justify-between"><div><p className="font-medium">{c.name}</p><p className="text-sm text-muted-foreground">{c.ageGroup}</p></div><div className="flex gap-2"><Button size="icon" variant="ghost" asChild><Link to={`/admin/classes/${c.id}`}><Settings className="h-4 w-4" /></Link></Button><Button size="icon" variant="ghost" onClick={()=>onDelete(c.id,c.name)}><Trash2 className="h-4 w-4" /></Button></div></CardContent></Card>)}</div></Layout>;
}
