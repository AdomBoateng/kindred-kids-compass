...
export const api = {
  ...
  getSettings: () => request<{ security: Record<string, unknown>; notifications: Record<string, boolean>; privacy: Record<string, boolean>; advanced: Record<string, unknown> }>("/api/v1/common/settings", { requiresAuth: true }),
  updateSettings: (section: "security" | "notifications" | "privacy" | "advanced", payload: Record<string, unknown>) => request(`/api/v1/common/settings/${section}`, { method: "PATCH", body: JSON.stringify(payload), requiresAuth: true }),
  getBirthdays: (days = 30, includeTeachers = false) => request<Array<{ id: string; full_name: string; class_name?: string; date_of_birth: string; days_until_birthday: number; person_type: "student" | "teacher" }>>(`/api/v1/common/birthdays?days=${days}${includeTeachers ? "&include_teachers=true" : ""}`, { requiresAuth: true }),
  ...
  sendBirthdaySmsReminders: () => request<{ sent: number }>("/api/v1/common/birthdays/remind-sms", { method: "POST", requiresAuth: true }),
  ...
  updateClass: (classId: string, payload: { name?: string; age_group?: string; description?: string }) => request(`/api/v1/admin/classes/${classId}`, { method: "PATCH", body: JSON.stringify(payload), requiresAuth: true }),
  ...
  unassignTeacherFromClass: (classId: string, teacherId: string) => request<{ deleted: boolean }>(`/api/v1/admin/classes/${classId}/teachers/${teacherId}`, { method: "DELETE", requiresAuth: true }),
  ...
  updateStudentAsAdmin: (studentId: string, payload: Record<string, unknown>) => request(`/api/v1/admin/students/${studentId}`, { method: "PATCH", body: JSON.stringify(payload), requiresAuth: true }),
